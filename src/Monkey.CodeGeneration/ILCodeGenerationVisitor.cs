...
{
    /// Genera CIL con Reflection.Emit y:
    /// - En .NET Core/5+/6+/8: ejecuta en memoria (no se puede guardar EXE).
    /// - En .NET Framework: puede también guardar EXE (RunAndSave).
    public class ILCodeGeneratorVisitor
    {
        ...
        // Ejecuta en memoria (válido en .NET moderno)
        ...
        // Guarda EXE (solo .NET Framework). En .NET moderno lanza NotSupportedException.
        ...
        private void BuildAssembly(IASTNode root, bool saveExe, string? exePath)
        {
...
#if NETFRAMEWORK
            // .NET Framework: soporta RunAndSave
            ...
            {
                // Crear método "Main" (punto de entrada) que llame a MonkeyProgram.main()
                ...
                var mainMb = _functions.TryGetValue("main", out var mb) ? mb : null;
                if (mainMb != null)
                    il.Emit(OpCodes.Call, mainMb);
                il.Emit(OpCodes.Ret);

                _asm.SetEntryPoint(eb, PEFileKinds.ConsoleApplication);
                _asm.Save(System.IO.Path.GetFileName(exePath)); // guarda en disco
            }
#endif
        }
        ...
        public void VisitNode(IASTNode? node)
        {
            ...
            switch (node)
            {
                ...
                case ExpressionStatement es:
                    ...
                    if (_currentMethod != null) _il.Emit(OpCodes.Pop);
                    break;
                ...
                default:
                    EmitExpr(node as IExpression); // por si cae aquí
                    break;
            }
        }
